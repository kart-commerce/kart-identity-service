...
/// <summary>
/// Strongly-typed mirror of contracts/message-bus-manifest.json — this service's entire
/// RabbitMQ topology (exchanges, queues, bindings, dead-letter/retry wiring). The manifest
/// is the single source of truth; nothing it describes may also be a hardcoded string
/// literal elsewhere in Infrastructure/Messaging.
/// </summary>
    ...
    IReadOnlyList<DeadLetterQueueDefinition> DeadLetterQueues)
{
    ...
    public QueueDefinition GetQueue(string name) =>
        Queues.FirstOrDefault(q => q.Name == name)
            ?? throw new InvalidOperationException(
                $"message-bus-manifest.json has no queue named '{name}'.");
    ...
    private PublishedEventDefinition PublishedEventFor(string eventType) =>
        PublishedEvents.FirstOrDefault(e => e.EventType == eventType)
            ?? throw new InvalidOperationException(
                $"message-bus-manifest.json has no publishedEvents entry for event type '{eventType}'.");
}
